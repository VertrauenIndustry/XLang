...
- Stage-2 self-hosting subset scaffold in [bootstrap/stage2](bootstrap/stage2).
  - Core builtins implemented:
  - `print(x)`, `println()`
  - `len(str)`, `clock_ms()`
  - `assert(bool)`, `panic(x)`
  - `max(i64,i64)`, `min(i64,i64)`, `abs(i64)`, `pow(i64,i64)`, `clamp(i64,i64,i64)`
  - `str(x)`, `int(x)`, `bool(x)`, `ord(str)`, `chr(i64)`
  - `contains(str,str)`, `starts_with(str,str)`, `ends_with(str,str)`
  - `find(str,str)` (first index or `-1`)
  - `split(str, sep, idx)` (returns `idx`-th piece or `""`)
  - `join(a, b, sep)` (returns `a + sep + b`)
  - `replace(str,str,str)`, `trim(str)`, `upper(str)`, `lower(str)`
  - `exit(i64)` (utility return code helper)
  - `input()`, `input(str)`
  - `argc()`, `argv(i64)`, `sleep_ms(i64)`
  - raw pointer helpers: `ptr_can_read(i64,i64)`, `ptr_can_write(i64,i64)`, `ptr_read8(i64)`, `ptr_write8(i64,i64)`, `ptr_read64(i64)`, `ptr_write64(i64,i64)`
  - compile-time/obfuscation helpers: `ct_hash(str)`, `ct_xor(str, key)`, `xor_decode(hex, key)`
